...
def preprocess_function(examples: Dataset, tokenizer: AutoTokenizer, col_name: str) ->BatchEncoding:
    ...
    """
    return tokenizer([" ".join(x) for x in examples[col_name]])


def group_texts(examples: Dataset, block_size: int) -> dict:
...
@hydra.main(config_path=f"{os.getcwd()}/conf", config_name="pretrain_config_bert.yaml")        
def main(cfg):
    """Run the pretraining of the Bert
    ...
    model = AutoModelForMaskedLM.from_pretrained(cfg.model.model_path)
    
    dataset = load_dataset("text", data_files={"train": [f"{cfg.dataset.base_path}/{cfg.dataset.data_file}"]})

    tokenized_dataset = dataset.map(
                    lambda x: preprocess_function(x, tokenizer, cfg.data_col_name),
                    batched=True,
    ...
    data_collator = DataCollatorForLanguageModeling(tokenizer=tokenizer, mlm_probability=cfg.model.mlm_probability)
    ...
    out_model.set_upload_destination(cfg.upload_destination)
